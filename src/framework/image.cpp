...
void Image::DrawTriangleInterpolated(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Color& c0, const Color& c1, const Color& c2, FloatImage* zbuffer, bool OCCLUSION, bool TEXTURE){
	ATE table;
	for (int i = 0; i < this->height; i++) table.push_back({ INT_MAX, INT_MIN });
	ScanLineBresenham(p0.x, p0.y, p1.x, p1.y, table);
	ScanLineBresenham(p1.x, p1.y, p2.x, p2.y, table);
...
void Image::DrawTriangleInterpolated(const sTriangleInfo& triangle, FloatImage* zbuffer, bool OCCLUSION, bool TEXTURE) {
	ATE table;
	for (int i = 0; i < this->height; i++) table.push_back({ INT_MAX, INT_MIN });
	...
	for (int i = 0; i < this->height; i++) {
		...
		for (int j = table[i].minX; j <= table[i].maxX; j++) {
			...
			if (zbuffer->GetPixel(j, i) < z) {
				if(OCCLUSION) zbuffer->SetPixel(j, i, z);

				Vector2 UV = triangle.uv0 * barycentric.x + triangle.uv1 * barycentric.y + triangle.uv2 * barycentric.z;
				Color pixelColor;
				
				if(&triangle.texture != nullptr && TEXTURE) pixelColor= triangle.texture->GetPixelSafe(UV.x * (triangle.texture->width - 1), UV.y * (triangle.texture->height - 1));
				else pixelColor = triangle.c0 * barycentric.x + triangle.c1 * barycentric.y + triangle.c2 * barycentric.z;
					
				this->SetPixelSafe(j, i, pixelColor);
			}
		}
	}
}
void Image::DrawMidline() {
	for (int i = this->width/2 - 5; i <= this->width/2 + 5; i++) {
		for (int j = 0; j < this->height; j++) {
			this->SetPixelSafe(i, j, Color::WHITE);
		}
	}
}