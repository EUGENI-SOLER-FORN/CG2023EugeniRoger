...
void main()
{
	...
	if(u_task == 0) {
		...
		rotated_uv = vec2(rotated_uv.x * cos(u_radius/3*u_time) - rotated_uv.y * sin(u_radius/3*u_time), rotated_uv.x * sin(u_radius/3*u_time) + rotated_uv.y * cos(u_radius/3*u_time)); 
		...
		gl_FragColor = texture2D(u_image, vec2(floor(v_uv.x * (u_pixelRate+1))/(u_pixelRate+1), floor(v_uv.y * (u_pixelRate+1))/(u_pixelRate+1)));
	}
	else if(u_task == 2){
