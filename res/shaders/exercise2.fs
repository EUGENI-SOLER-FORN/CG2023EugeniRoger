...
void main()
{
	...
	if(u_task == 0) gl_FragColor = vec4((pixelCol.x + pixelCol.y + pixelCol.z)/3.0);
		...
		blurred += texture2D(u_image, v_uv + vec2(1.0,0.0)/u_width);
		blurred += texture2D(u_image, v_uv + vec2(2.0,0.0)/u_width);
		blurred += texture2D(u_image, v_uv + vec2(3.0,0.0)/u_width);
		blurred += texture2D(u_image, v_uv + vec2(4.0,0.0)/u_width);

		blurred += texture2D(u_image, v_uv + vec2(1.0,0.0)/u_height);
		blurred += texture2D(u_image, v_uv + vec2(2.0,0.0)/u_height);
		blurred += texture2D(u_image, v_uv + vec2(3.0,0.0)/u_height);
		blurred += texture2D(u_image, v_uv + vec2(4.0,0.0)/u_height);
